  ...
  Mm calc_dynam_params(Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm TPY, Mm TPX, Mm code, i_o_t, Mm& SNR__o, Mm& SFDR__o, \
     Mm& SNRFS__o, Mm& SINAD__o, Mm& THD__o, Mm& HD__o, Mm& ENOB__o, Mm& ENOBFS__o, Mm& Pn_dB__o) {
    begin_scope
    fclk.setname("fclk"); numbit.setname("numbit"); NFFT.setname("NFFT"); V.setname("V"); TPY.setname("TPY"); TPX.setname( \
      "TPX"); code.setname("code"); 
    dMm(SNR); dMm(SFDR); dMm(SNRFS); dMm(SINAD); dMm(THD); dMm(HD); dMm(ENOB); dMm(ENOBFS); dMm(Pn_dB); dMm(fid);  \
      dMm(ADout); dMm(ad_len_N); dMm(maxADout); dMm(AmpMax); dMm(t1); dMm(AmpMin); dMm(t2); dMm(Vpp); dMm(ADout_w);  \
      dMm(AA); dMm(ad_len); dMm(ADout_spect); dMm(ADout_dB); dMm(maxdB); dMm(fin); dMm(freq_fin); dMm(n); dMm(data_ref) \
      ...
      dMm(har_num); dMm(har_num_calc_dynam_params_v0); dMm(tone); dMm(har_peak); dMm(har_bin); dMm(spectP_temp); dMm( \
      i_); dMm(i_calc_dynam_params_v1); dMm(disturb_len); dMm(spectP_disturb); dMm(Harbin_disturb); dMm(findSpac);  \
      dMm(findSpan); dMm(findStart); dMm(i_calc_dynam_params_v2); dMm(spectP_disturb_peak); dMm(num); dMm(array_flag) \
      ; dMm(j_); dMm(j_calc_dynam_params_v3); dMm(k); dMm(k_calc_dynam_params_v4); dMm(spectP_disturb_temp); dMm(Harbin_disturb_temp) \
      ; dMm(Ph_disturb); dMm(Ph_disturb_dB); dMm(Fn_disturb); dMm(i_calc_dynam_params_v5); dMm(Pd_disturb); dMm(Pd_disturb_dB) \
      ; dMm(Pd); dMm(Pd_dB); dMm(Pn); dMm(Vin); dMm(NFFT_txt); dMm(FRQ_txt); dMm(FFT_txt); dMm(FREQ_txt); dMm(DBFS_txt) \
      ; dMm(SNR_txt); dMm(SFDR_txt); dMm(i_calc_dynam_params_v6); dMm(VPP_txt); 
    ...
    // TPX = 0.01;
    // code = 1;
    fid = fopen(TM("SNR测试_A通道_20090105053713.txt"));
    //;
    //fid = fopen('SNR测试_A通道_20090105053713.txt');
    //fid = fopen('V1.0_TB.txt');
    ADout = fscanf(fid,TM("%d"));
    if (istrue(code==1.0)) {
    ...
    //refdB
    n = colon(0.0,1.0,ad_len_N-1.0);
    
    data_ref = V/2.0*sin(n*(freq_fin)/fclk*2.0*pi);
    data_ref_w = times(ctranspose(data_ref),chebwin(ad_len_N,200.0));
    ...
    har_num_calc_dynam_params_v0 = colon(1.0,1.0,11.0); int har_num_calc_dynam_params_i0;
    for (har_num_calc_dynam_params_i0=0;har_num_calc_dynam_params_i0<har_num_calc_dynam_params_v0.cols();har_num_calc_dynam_params_i0++) \
       {
      forelem(har_num,har_num_calc_dynam_params_v0,har_num_calc_dynam_params_i0);
    ...
    i_calc_dynam_params_v1 = colon(2.0,1.0,10.0); int i_calc_dynam_params_i1;
    for (i_calc_dynam_params_i1=0;i_calc_dynam_params_i1<i_calc_dynam_params_v1.cols();i_calc_dynam_params_i1++)  \
      {
      forelem(i_,i_calc_dynam_params_v1,i_calc_dynam_params_i1);
    ...
    i_calc_dynam_params_v2 = colon(findStart,findSpac,ad_len/2.0); int i_calc_dynam_params_i2;
    for (i_calc_dynam_params_i2=0;i_calc_dynam_params_i2<i_calc_dynam_params_v2.cols();i_calc_dynam_params_i2++)  \
      ...
      if (istrue(array_flag==1.0)) {
        j_calc_dynam_params_v3 = colon(1.0,1.0,disturb_len-2.0); int j_calc_dynam_params_i3;
        for (j_calc_dynam_params_i3=0;j_calc_dynam_params_i3<j_calc_dynam_params_v3.cols();j_calc_dynam_params_i3++) \
           {
          forelem(j_,j_calc_dynam_params_v3,j_calc_dynam_params_i3);
          k_calc_dynam_params_v4 = colon(1.0,1.0,(disturb_len-j_)); int k_calc_dynam_params_i4;
          for (k_calc_dynam_params_i4=0;k_calc_dynam_params_i4<k_calc_dynam_params_v4.cols();k_calc_dynam_params_i4++) \
             {
            forelem(k,k_calc_dynam_params_v4,k_calc_dynam_params_i4);
    ...
    i_calc_dynam_params_v5 = colon(1.0,1.0,disturb_len); int i_calc_dynam_params_i5;
    for (i_calc_dynam_params_i5=0;i_calc_dynam_params_i5<i_calc_dynam_params_v5.cols();i_calc_dynam_params_i5++)  \
      {
      forelem(i_,i_calc_dynam_params_v5,i_calc_dynam_params_i5);
    ...
    text((CL(Fh(10.0)*fclk),ADout_dB(Harbin(10.0))-ref_dB+2.0,TM("10"),TM("VerticalAlignmen"),TM("bottom"),TM("Color") \
      ,TM("r")));
    hold(TM("on"));
    i_calc_dynam_params_v6 = colon(0.0,1.0,disturb_len/2.0); int i_calc_dynam_params_i6;
    ...
    hold(TM("on"));
    axis((BR(0.0),TPX,-TPY,TPY));
    // text(t1,AmpMax,num2str(AmpMax),'VerticalAlignmen','bottom');
    // text(t1,AmpMin,num2str(AmpMin),'VerticalAlignmen','top')
    display( text((CL(TPX*0.85),0.9*TPY,VPP_txt,TM("HorizontalAlignment"),TM("left"),TM("Color"),TM("r"))) );
    display( text((CL(TPX*0.85),0.8*TPY,FRQ_txt,TM("HorizontalAlignment"),TM("left"),TM("Color"),TM("r"))) );
    ...
    // function exit code
    fclk.setname(NULL); numbit.setname(NULL); NFFT.setname(NULL); V.setname(NULL); TPY.setname(NULL); TPX.setname( \
      NULL); code.setname(NULL); 
  ...
  Mm calc_dynam_params(Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm TPY, Mm TPX, Mm code) {
    ...
    dMm(SNR__o); dMm(SFDR__o); dMm(SNRFS__o); dMm(SINAD__o); dMm(THD__o); dMm(HD__o); dMm(ENOB__o); dMm(ENOBFS__o) \
      ; dMm(Pn_dB__o); 
    calc_dynam_params(fclk, numbit, NFFT, V, TPY, TPX, code, i_o, SNR__o, SFDR__o, SNRFS__o, SINAD__o, THD__o, HD__o, \
       ENOB__o, ENOBFS__o, Pn_dB__o);
  ...
  Mm calc_dynam_params(Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm TPY, Mm TPX, Mm code, i_o_t, Mm& SNR__o, Mm& SFDR__o, \
     Mm& SNRFS__o, Mm& SINAD__o, Mm& THD__o) {
    begin_scope
    double old_nargin=nargin_val; nargin_val=7.0; nargin_set=1;
  ...
  Mm calc_dynam_params(Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm TPY, Mm TPX, Mm code, i_o_t, Mm& SNR__o, Mm& SFDR__o, \
     Mm& SNRFS__o, Mm& SINAD__o, Mm& THD__o, Mm& HD__o) {
    begin_scope
    double old_nargin=nargin_val; nargin_val=7.0; nargin_set=1;
  ...
  Mm calc_dynam_params(Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm TPY, Mm TPX, Mm code, i_o_t, Mm& SNR__o, Mm& SFDR__o, \
     Mm& SNRFS__o, Mm& SINAD__o, Mm& THD__o, Mm& HD__o, Mm& ENOB__o) {
    begin_scope
    double old_nargin=nargin_val; nargin_val=7.0; nargin_set=1;
  ...
  Mm calc_dynam_params(Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm TPY, Mm TPX, Mm code, i_o_t, Mm& SNR__o, Mm& SFDR__o, \
     Mm& SNRFS__o, Mm& SINAD__o, Mm& THD__o, Mm& HD__o, Mm& ENOB__o, Mm& ENOBFS__o) {
    begin_scope
    double old_nargin=nargin_val; nargin_val=7.0; nargin_set=1;
