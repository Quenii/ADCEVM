...
numbit=16;
fclk = 80e6;
% fid = fopen('V1_0_TB.txt');% V1_0_TB.txt 2m5_sig2.txt
% ADout = fscanf(fid,'%d') / 2^(numbit-1);
ADout = data(1:numpt) / 2^(numbit-1);
%AD amp deviation
maxADout = max(abs(ADout));
diffADout = 0.9999 / maxADout;
diffADoutdb = 20 * log10(diffADout);
...
span_s = max(round(ad_len/200),5);
%Determine power spectrum
spectP=(abs(ADout_spect)).*(abs(ADout_spect)); 
%Find DC offset power 
Pdc=sum(spectP(1:spanh_har)); 
...
% legend('1st','2nd','3rd','4th','5th','6th','7th','8th','9th');
% hold on;
% plot(Fh(10)*fclk,ADout_dB(Harbin(10))-maxdB,'ro',Fh(11)*fclk,ADout_dB(Harbin(11))-maxdB,'rx',Fh(12)*fclk,ADout_dB(Harbin(12))-maxdB,'r+',Fh(13)*fclk,ADout_dB(Harbin(13))-maxdB,'r*', Fh(14)*fclk,ADout_dB(Harbin(14))-maxdB,'rs',Fh(15)*fclk,ADout_dB(Harbin(15))-maxdB,'rd',Fh(16)*fclk,ADout_dB(Harbin(16))-maxdB,'rd');
% 
% for i = 1:disturb_len
%     hold on;
