...
// isInverse表示是否为逆变换
void CAdcTestPlatView::FFT(double * data, int n, bool isInverse )
{
    int mmax, m, j, step, i;
    double temp;
...
void CAdcTestPlatView::DrawData( void )
{
...
/*
		// 查找批测试视图
		CTestSetBatchView* pTestSetBatchView = NULL;			
		POSITION pos = pDoc->GetFirstViewPosition();
		while ( pos != NULL )
		{
			CView* pView = pDoc->GetNextView( pos );			
			...
			if ( pView->IsKindOf(RUNTIME_CLASS(CTestSetBatchView)) )
			{
				pTestSetBatchView = (CTestSetBatchView*)pView;
				break;
			}
		}
		// 分析测试结果, 在开始测试之后才显示
		if ( pTestSetBatchView != NULL && !(pTestSetBatchView->m_bTestBegin) )
		{
			// 获取阈值
			pTestSetBatchView->UpdateData( TRUE );
			dSnrThreshold = atof( pTestSetBatchView->m_strSNR );
			dSfdrThreshold = atof( pTestSetBatchView->m_strSFDR );
			dSinadThreshold = atof( pTestSetBatchView->m_strSINAD );
			dBackNoiseThreshold = atof( pTestSetBatchView->m_strBackNoise );
			dChanDisturbThreshold = atof( pTestSetBatchView->m_strChanDisturb );
			...
			if ( pDoc->m_bTestSNR )
			{
				...
				{
					// 有一个不满足，则失败
					if ( pDoc->m_daResultSNR[i] < dSnrThreshold )
						break;
				}
				// 通过
				if ( i >=  CHANNEL_NUM )
				{				
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SNR ))->SetBitmap( pTestSetBatchView->m_bmpOk );
				}
				// 未通过
				else
				{			
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SNR ))->SetBitmap( pTestSetBatchView->m_bmpCross );
					bPass = FALSE;
				}
			}
			else
			{
				((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SNR ))->SetBitmap( pTestSetBatchView->m_bmpBack );
			}
			...
			if ( pDoc->m_bTestSINAD )
			{
				...
				{
					// 有一个不满足，则失败
					if ( pDoc->m_daResultSINAD[i] < dSinadThreshold )
						break;
				}
				// 通过
				if ( i >=  CHANNEL_NUM )
				{				
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SINAD ))->SetBitmap( pTestSetBatchView->m_bmpOk );
				}
				// 未通过
				else
				{			
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SINAD ))->SetBitmap( pTestSetBatchView->m_bmpCross );
					bPass = FALSE;
				}
			}
			else
			{
				((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SINAD ))->SetBitmap( pTestSetBatchView->m_bmpBack );
			}
			...
			if ( pDoc->m_bTestSFDR )
			{
				...
				{
					// 有一个不满足，则失败
					if ( pDoc->m_daResultSFDR[i] < dSfdrThreshold )
						break;
				}
				// 通过
				if ( i >=  CHANNEL_NUM )
				{				
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SFDR ))->SetBitmap( pTestSetBatchView->m_bmpOk );
				}
				// 未通过
				else
				{			
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SFDR ))->SetBitmap( pTestSetBatchView->m_bmpCross );
					bPass = FALSE;
				}
			}
			else
			{
				((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_SFDR ))->SetBitmap( pTestSetBatchView->m_bmpBack );
			}
			...
			if ( pDoc->m_bTestBackNoise )
			{
				...
				{
					// 有一个不满足，则失败
					if ( pDoc->m_daResultBackNoise[i] > dBackNoiseThreshold )
						break;
				}
				// 通过
				if ( i >=  CHANNEL_NUM )
				{				
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_BACK_NOISE ))->SetBitmap( pTestSetBatchView->m_bmpOk );
				}
				// 未通过
				else
				{			
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_BACK_NOISE ))->SetBitmap( pTestSetBatchView->m_bmpCross );
					bPass = FALSE;
				}
			}
			else
			{
				((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_BACK_NOISE ))->SetBitmap( pTestSetBatchView->m_bmpBack );
			}
			...
			if ( pDoc->m_bTestChanDisturb )
			{
				...
				{
					// 有一个不满足，则失败
					if ( pDoc->m_daResultChanDisturb[i] > dChanDisturbThreshold )
						break;
				}
				// 通过
				if ( i >=  CHANNEL_NUM )
				{				
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_CHAN_DISTURB ))->SetBitmap( pTestSetBatchView->m_bmpOk );
				}
				// 未通过
				else
				{			
					((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_CHAN_DISTURB ))->SetBitmap( pTestSetBatchView->m_bmpCross );
					bPass = FALSE;
				}
			}
			else
			{
				((CStatic*)pTestSetBatchView->GetDlgItem( IDC_PIC_CHAN_DISTURB ))->SetBitmap( pTestSetBatchView->m_bmpBack );
			}
			
			// 有一个未通过，对错误计数，否则正确计数
			if ( bPass )
			{
				pTestSetBatchView->m_nPassCount++;
				pTestSetBatchView->m_strPassCount.Format( "%d", pTestSetBatchView->m_nPassCount );
			}
			else
			{
				pTestSetBatchView->m_nErrorCount++;
				pTestSetBatchView->m_strErrorCount.Format( "%d", pTestSetBatchView->m_nErrorCount );
			}
			pTestSetBatchView->UpdateData( FALSE );
		}
*/
	}
