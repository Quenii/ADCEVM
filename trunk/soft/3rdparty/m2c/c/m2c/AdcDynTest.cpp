  ...
  Mm AdcDynTest(Mm ADout, Mm fclk, Mm numbit, Mm NFFT, Mm V, Mm code, i_o_t, Mm& SNR__o, Mm& SINAD__o, Mm& SFDR__o, \
    ...
    ADout.setname("ADout"); fclk.setname("fclk"); numbit.setname("numbit"); NFFT.setname("NFFT"); V.setname("V");  \
      code.setname("code"); 
    dMm(SNR); dMm(SINAD); dMm(SFDR); dMm(ENOB); dMm(HD); dMm(y); dMm(Vpp); dMm(Vin); dMm(THD); dMm(ad_len_N); dMm( \
      maxADout); dMm(AmpMax); dMm(t1); dMm(AmpMin); dMm(t2); dMm(ADout_w); dMm(AA); dMm(ad_len); dMm(ADout_spect);  \
      dMm(ADout_dB); dMm(maxdB); dMm(fin_v); dMm(fin); dMm(freq_fin); dMm(data_ref); dMm(n); dMm(n_AdcDynTest_v0);  \
      dMm(data_ref_w); dMm(data_ref_spect); dMm(data_ref_dB); dMm(ref_dB); dMm(span); dMm(spanh_har); dMm(span_s);  \
      dMm(spectP); dMm(Pdc); dMm(Pdc_dB); dMm(l); dMm(u); dMm(Ps); dMm(Ps_dB); dMm(Fh); dMm(Ph); dMm(Harbin); dMm(Ph_dB) \
      ; dMm(har_num); dMm(har_num_AdcDynTest_v1); dMm(tone); dMm(har_peak); dMm(har_bin); dMm(spectP_temp); dMm(i_) \
      ; dMm(i_AdcDynTest_v2); dMm(disturb_len); dMm(spectP_disturb); dMm(Harbin_disturb); dMm(findSpac); dMm(findSpan) \
      ; dMm(findStart); dMm(i_AdcDynTest_v3); dMm(spectP_disturb_peak); dMm(num); dMm(array_flag); dMm(jj); dMm(jj_AdcDynTest_v4) \
      ; dMm(k); dMm(k_AdcDynTest_v5); dMm(spectP_disturb_temp); dMm(Harbin_disturb_temp); dMm(Ph_disturb); dMm(Ph_disturb_dB) \
      ; dMm(Fn_disturb); dMm(i_AdcDynTest_v6); dMm(Pd_disturb); dMm(Pd_disturb_dB); dMm(Pd); dMm(Pd_dB); dMm(Pn); dMm( \
      Pn_dB); dMm(SNRFS); dMm(ENOBFS); 
