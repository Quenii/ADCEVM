...
#include "stdafx.h"
#include "gkhy\mfcplus\TextProgressCtrl.hpp"
#include "gkhy\mfcplus\MemDC.hpp"
...
CTextProgressCtrl::CTextProgressCtrl()
{
	...
	m_colBoarder		= GetSysColor(COLOR_GRADIENTINACTIVECAPTION); // by roc
...
LRESULT CTextProgressCtrl::OnGetText(WPARAM wParam, LPARAM lParam)
{
	...
	if (!wcsncpy_s(szText, cchTextMax, m_strText, m_strText.GetLength()))
		...
		return qMin(cchTextMax, (UINT)m_strText.GetLength());
}
...
void CTextProgressCtrl::OnPaint() 
{
    ...
    CRect /*LeftRect, RightRect, */ HotRect, ColdRect, ClientRect;
    GetClientRect(ClientRect);
	dc.FillSolidRect(ClientRect, m_colBoarder);//COLOR_BACKGROUND));// GetForeColour());
	...
	ColdRect = HotRect = ClientRect;
	switch(m_Direction)
	{
	case DIR_LTOR:
		{
			HotRect.right = HotRect.left + int(HotRect.Width() * Fraction);
			ColdRect.left = HotRect.right;
			break;
		}

	case DIR_RTOL:
		{
			HotRect.left = HotRect.right - int(HotRect.Width() * Fraction);
			ColdRect.right = HotRect.left;
			break;
		}

	case DIR_TTOB:
		{
			HotRect.bottom = HotRect.top + int(HotRect.Height() * Fraction);
			ColdRect.top = HotRect.bottom;
			break;
		}

	case DIR_BTOT:
		{
			HotRect.top = HotRect.bottom - int(HotRect.Height() * Fraction);
			ColdRect.bottom = HotRect.top;
			break;
		}
	...
	dc.FillSolidRect(HotRect, m_colFore);
    dc.FillSolidRect(ColdRect, m_colBk);
    ...
    if (m_bShowText)
    {
        ...
        CRgn rgn;
        rgn.CreateRectRgn(HotRect.left, HotRect.top, HotRect.right, HotRect.bottom);
        ...
        rgn.DeleteObject();
        rgn.CreateRectRgn(ColdRect.left, ColdRect.top, ColdRect.right, ColdRect.bottom);
        dc.SelectClipRgn(&rgn);
...
void CTextProgressCtrl::SetDirection(Direction dir)
{
	m_Direction = dir;
}