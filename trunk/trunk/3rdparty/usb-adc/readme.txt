    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_libalgo.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * libalgo.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * libalgo.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * libalgo (application created by MATLAB Compiler)
        * run_libalgo.sh (shell script that can be run to 
    ...
    All Platforms:
        * libalgo.dll (shared library, file extension varies by 
          platform)
        * libalgo.h (library header file)
        * libalgo.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * libalgo.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * libalgo.xml (documentation files)
        * libalgo.pdb (program database file - if DEBUG option 
          selected)
        * libalgo.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * libalgo.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * libalgo.ctf (Component Technology File archive, contains 
          compiled M-code)
        * libalgo.xla (Optionally created and placed in
