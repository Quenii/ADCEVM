...
#include "stdafx.h"
#include "gkhy/mfcplus/MMTimer.hpp"
...
bool CMMTimer::Start(UINT Delay, UINT Resolution)
{
    ...
    if(!m_RunningFlag)
    {
        ...
        if((m_Id = ::timeSetEvent(Delay, Resolution, TimeProc,
            reinterpret_cast<DWORD_PTR>(this), TIME_PERIODIC)) != NULL)
        {            
...
// occurs.
void CALLBACK CMMTimer::TimeProc(UINT uTimerID, UINT uMsg, DWORD_PTR dwUser, 
