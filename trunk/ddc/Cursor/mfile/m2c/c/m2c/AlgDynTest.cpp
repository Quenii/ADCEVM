  ...
  Mm AlgDynTest(Mm data1, Mm data2, Mm numpt, Mm fclk, Mm numbit, Mm r, i_o_t, Mm& SNR__o, Mm& SINAD__o, Mm& SFDR__o, \
    ...
    dMm(SNR); dMm(SINAD); dMm(SFDR); dMm(ENOB); dMm(all_); dMm(fpga_i); dMm(fpga_q); dMm(V); dMm(NFFT); dMm(TPY);  \
      ...
      Ph_dB_1); dMm(har_num); dMm(har_num_AlgDynTest_v1); dMm(tone); dMm(har_peak); dMm(har_bin); dMm(har_peak_1);  \
      dMm(har_bin_1); dMm(spectP_temp); dMm(i_); dMm(i_AlgDynTest_v2); dMm(disturb_len); dMm(spectP_disturb); dMm(Harbin_disturb) \
      ; dMm(findSpac); dMm(findSpan); dMm(findStart); dMm(i_AlgDynTest_v3); dMm(spectP_disturb_peak); dMm(num); dMm( \
      array_flag); dMm(jj); dMm(jj_AlgDynTest_v4); dMm(k); dMm(k_AlgDynTest_v5); dMm(spectP_disturb_temp); dMm(Harbin_disturb_temp) \
      ; dMm(Ph_disturb); dMm(Ph_disturb_dB); dMm(Fn_disturb); dMm(i_AlgDynTest_v6); dMm(Pd_disturb); dMm(Pd_disturb_dB) \
