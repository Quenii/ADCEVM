...
spectP = (abs(ADout_spect).^2);  % spectP = (abs(ADout_spect)) .* (abs(ADout_spect)); 
%Find DC offset power 
Pdc=sum(spectP(ad_len/2-span:ad_len/2+span)); 
Pdc_dB=sum(ADout_dB(ad_len/2-span:ad_len/2+span));
%Extract overall signal power 

Ps =sum(spectP(max(fin - span_s, 1) : min(fin + span_s, length(spectP) )) );
Ps_dB =sum(ADout_dB(max(fin - span_s, 1) : min(fin + span_s, length(ADout_dB))));
...
for i = 1:disturb_len
 Ph_disturb=[Ph_disturb sum(spectP(max(1, Harbin_disturb(i)-spanh_har) : min(length(spectP), Harbin_disturb(i)+spanh_har)))];
 Ph_disturb_dB = [Ph_disturb_dB sum(ADout_dB(max(1, Harbin_disturb(i)-spanh_har) : min(length(ADout_dB), Harbin_disturb(i)+spanh_har)))];
end
